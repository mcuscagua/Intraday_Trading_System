...
#             return k
    ...
    def stream_price2(self, epics, multiple = False):
        """
        takes an string of tickers for oanda to stream. For multiple, set parameter
        multiple as True
        """
        ...
        for i in resp.iter_lines(1):
          ...
          if (var['type'] != 'HEARTBEAT'):
            ...
            if var['instrument'] not in dicf['inst'].keys():# or dicf['inst'][var['instrument']]=={}:
                dicf['inst'][var['instrument']] ={0 :{'open':float(var['bids'][0]['price']),'high':float(var['bids'][0]['price']),
                                                   'low':float(var['bids'][0]['price']),'close':float(var['bids'][0]['price']),
                                                   'hora':var['time']}}

            # Crea si existe el instrumento pero ya no cabe en la vela
            elif str(pd.to_datetime(var['time']) - pd.to_datetime(dicf['inst'][var['instrument']][max(dicf['inst'][var['instrument']].keys())]['hora'])).split(' ')[2].split(':')[1] != '00':

                dicf['inst'][var['instrument']][1+max(dicf['inst'][var['instrument']].keys())] = {'open':float(var['bids'][0]['price']),'high':float(var['bids'][0]['price']),
                                       'low':float(var['bids'][0]['price']),'close':float(var['bids'][0]['price']),
                                       'hora':var['time']}
            ...
            # Actualiza si aún cabe en la vela
                ...
                #try:
                dicf['inst'][var['instrument']][max(dicf['inst'][var['instrument']].keys())]['close'] = float(var['bids'][0]['price'])

                if float(var['bids'][0]['price']) > dicf['inst'][var['instrument']][max(dicf['inst'][var['instrument']].keys())]['high']:
                    dicf['inst'][var['instrument']][max(dicf['inst'][var['instrument']].keys())]['high'] = float(var['bids'][0]['price'])
                elif float(var['bids'][0]['price']) < dicf['inst'][var['instrument']][max(dicf['inst'][var['instrument']].keys())]['low']:
                    dicf['inst'][var['instrument']][max(dicf['inst'][var['instrument']].keys())]['low'] = float(var['bids'][0]['price'])
                #except:
                #    del dicf['inst'][var]

            # Convertir 10 minutos en csv, y volver a empezar
            #if max(dicf['inst'][var['instrument']].keys())% 10==0 and max(dicf['inst'][var['instrument']].keys())!=0:
            if len(dicf['inst'][var['instrument']].keys()) == 11:
                
                df = pd.DataFrame(columns=['open','high','low','close','inst'],index=[1],data=[[np.nan]*5])
                for var in dicf['inst'].keys():
                    tdf = pd.DataFrame.from_dict(dicf['inst'][var],orient='index')
                    tdf['inst'] = var
                    df = pd.concat([df,tdf])
                ...
                try:
                    for var in dicf['inst'].keys():
                        del dicf['inst'][var][min(list(dicf['inst'][var].keys()))]
                except:
                    del dicf['inst'][var]
                print(df)
            #break
    ...
    def stream_price3(self, epics, multiple = False):
        """
        takes an string of tickers for oanda to stream. For multiple, set parameter
        multiple as True
        """
        ...
        for i in resp.iter_lines(1):
          ...
          if (var['type'] != 'HEARTBEAT'):
            ...
            # Convertir 10 minutos en csv, y volver a empezar
            #if max(dicf['inst'][var['instrument']].keys())% 10==0 and max(dicf['inst'][var['instrument']].keys())!=0:
            if datetime.datetime.now().time() > pd.to_datetime('2020-04-16 23:59:59').time():
